...
*local terminals, Kubernetes dashboards, System monitoring — built for home lab engineers.*
...
- Password auto-save — detects SSH password prompts in the terminal, saves credential only after confirmed successful login (never on a failed attempt)
...
- Two-way sync — saving a session mirrors its password to the Git Bash credential store and vice versa
...
- No filesystem navigation needed — all credential management from inside the app

### Kubernetes Dashboard
- **Session selector dropdown** — choose which saved session runs `kubectl` directly from the dashboard header
- **Silent background SSH connection** — connects to master node without opening a terminal tab or switching views
...
### System Monitor
- Accessible from the sidebar — dedicated **📊 System Monitor** entry under K8s Dashboard
...
- Auto-starts local monitoring when opened from sidebar
...
# Ensure setuptools is installed (required for Python 3.12+)
python -m pip install setuptools
...
# Or build portable exe (no installer)
...
- Legacy plaintext passwords are automatically migrated and encrypted on first launch
...
### Password saved even when login failed
Fixed in latest version — credentials are only saved after the shell prompt is detected (`$`, `#`, `>`), never on `Permission denied` or `authentication failed` responses.
